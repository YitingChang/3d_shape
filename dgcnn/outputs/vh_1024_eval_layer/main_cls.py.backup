...
where it was trained for point cloud classification using the ModelNet40 dataset. 

Here, it was modified to process the visual haptic objects and to obtain 
(1) classification and (2) features from a hidden layer.  
...
from data import ModelNet40, vhObject
...
def test_layer_modelnet(args, io):
    test_loader = DataLoader(ModelNet40(partition='test', num_points=args.num_points),
                             batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    ...
    hook_handle = model.linear1.register_forward_hook(hook_fn)  # For example, second-to-last layer: linear2
    ...
    np.save('layer_outputs/ModelNet40/test_labels.npy', test_labels)
    ...
    # np.save('second_to_last_layer_output.npy', second_last_outputs)
    np.save('layer_outputs/ModelNet40/third-to-last_layer_outputs.npy', test_layer_outputs)
